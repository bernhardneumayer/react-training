/**
 * Session 3.3 - useRef Hook - SOLUTIONS
 *
 * Complete working solutions for all useRef exercises
 * Demonstrates:
...
// EXERCISE 1 SOLUTION: Focus input on mount
...
// EXERCISE 2 SOLUTION: Manual focus control
...
// EXERCISE 3 SOLUTION: Scroll to element
...
// EXERCISE 4 SOLUTION: Store interval ID (useRef for non-rendering values)
export function StopwatchWithRef() {
  ...
  const start = () => {
    ...
    setIsRunning(true)
    // Store interval ID in intervalRef so we can clear it later
  ...
  const stop = () => {
    setIsRunning(false)
    // Clear the interval using the stored intervalRef
...
// EXERCISE 5 SOLUTION: Track previous value
function usePrevious<T>(value: T): T | undefined {
  const ref = useRef<T>()

  useEffect(() => {
    // Store current value in ref after render
    // This happens AFTER the render, so ref.current still has the previous value
    // during the current render
    ref.current = value
  }, [value])

  // Return the previous value (which is still in ref.current during current render)
  return ref.current
}
...
// EXERCISE 6 SOLUTION: Track render count
export function RenderCounter() {
  const [count, setCount] = useState(0)
  const renderCount = useRef(0)

  // This runs on every render
  // Incrementing ref.current does NOT trigger a re-render
  renderCount.current++
...
// EXERCISE 7 SOLUTION: useRef vs useState comparison
...
// EXERCISE 8 SOLUTION: Video player controls
export function VideoPlayer() {
  const videoRef = useRef<HTMLVideoElement>(null)
  const [isPlaying, setIsPlaying] = useState(false)

  const handlePlay = () => {
    // Use ref to access the native video element and call play()
    videoRef.current?.play()
    setIsPlaying(true)
  }

  const handlePause = () => {
    // Use ref to access the native video element and call pause()
  ...
  const handleRestart = () => {
    if (videoRef.current) {
      // Reset currentTime to 0 and play
...
// EXERCISE 9 SOLUTION: Measure element size
...
// EXERCISE 10 SOLUTION: Click outside detector
export function ClickOutsideDemo() {
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      // contains() returns true if the target is inside the dropdown
...
// EXERCISE 11 SOLUTION: Uncontrolled form with refs
export function UncontrolledForm() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Get values from refs (not from state!)
    // This is an "uncontrolled" form - React doesn't track the values
    const formData = {
    ...
    // Clear the form by directly manipulating the DOM
